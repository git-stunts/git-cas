...
preflight symbolic refs before no-dereference updates. Checked deletion probes
ref type again after a transaction conflict, so an observed dangling symbolic
ref is a release conflict rather than an idempotent miss.
...
structural compatibility. `get()` retains its existing complete target
...
6,310 tests across Node, Bun, and Deno. Its npm dry run contained 242 files
(746,100 bytes packed; 2,051,091 bytes unpacked). CI, self-review, Code Rabbit,
