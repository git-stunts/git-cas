...
- **`framed` default encrypted writes** — new encrypted stores now default to `framed`, gaining per-frame AAD binding by default. `encryption.frameBytes` implies framed mode even when `scheme` is omitted.
