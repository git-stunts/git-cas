...
import { GIT_CAS_THEME, shellRule, themeText } from './theme.js';
...
function uiColor(ctx, tokenName) {
  return ctx.theme.theme.ui[tokenName]?.hex ?? GIT_CAS_THEME.ui[tokenName].hex;
}
...
function blank(ctx, width, height) {
  return createSurface(Math.max(1, width), Math.max(1, height), {
    char: ' ',
    bg: uiColor(ctx, 'canvas'),
  });
}

function verticalRail(ctx, height) {
  return textSurface(
    Array.from({ length: Math.max(1, height) }, () =>
      themeText(ctx, '│', { tone: 'accent', bold: true })
    ).join('\n'),
    1,
    height
  );
...
function panel({ title, body, width, height, ctx, active = false }) {
  ...
  if (active) {
    placed.blit(verticalRail(ctx, safeHeight), 0, 0);
  }
  return placed;
}
...
function workspaceTabs(model, ctx) {
  ...
  return tabs
    .map(([id, label]) => {
      ...
      return themeText(ctx, active ? `▌ ${label}` : `  ${label}`, {
...
function renderWorkspaceSurface(model, deps, width) {
  const left = workspaceTabs(model, deps.ctx);
  const right = model.gitBranch
    ? themeText(deps.ctx, model.gitBranch, { tone: 'info' })
    : themeText(deps.ctx, model.status, { tone: 'secondary' });
  const first = statusBarSurface({ left: ` ${left}`, right, width });
  ...
  const search = `${themeText(deps.ctx, 'search', { tone: 'accent' })} ${model.filterText || 'all assets'}`;
  const detailStatus = selectedShaFooter(model);
  const third = statusBarSurface({
    left: ` ${search}`,
    right: model.error
      ? themeText(deps.ctx, model.error, { tone: 'danger' })
      : themeText(deps.ctx, detailStatus ?? '', { tone: 'info' }),
    width,
  });
...
export function renderDashboard(model, deps, options = {}) {
  const renderModel = {
    ...model,
    columns: options.width ?? model.columns,
    rows: options.height ?? model.rows,
  };
  if (renderModel.phase === 'title' || renderModel.phase === 'password') {
    return renderTitleScreen(renderModel, deps);
  }

  const width = Math.max(1, renderModel.columns);
  const height = Math.max(1, renderModel.rows);
  const screen = createSurface(width, height, {
    char: ' ',
    bg: uiColor(deps.ctx, 'canvas'),
  });
  const header = renderWorkspaceSurface(renderModel, deps, width);
  const bodyHeight = Math.max(1, height - header.height);
  const body = renderBody(renderModel, deps, { width: Math.max(1, width - 1), height: bodyHeight });

  screen.blit(
    vstackSurface(header, hstackSurface(0, verticalRail(deps.ctx, bodyHeight), body)),
    0,
    0
  );
  renderPageOverlay(renderModel, deps, { screen, width, height });
  return screen;
}
...
function renderTitleScreen(model, deps) {
  ...
  screen.blit(
    canvas(width, height, organicFlowShader, {
  ...
  screen.blit(
    titlePanel,
    Math.max(0, Math.floor((width - titlePanel.width) / 2)),
    Math.max(0, Math.floor((height - titlePanel.height) / 3))
  );
  return screen;
}
...
function titlePhaseSurfaces(model, ctx, width) {
  ...
  if (model.promptEnter) {
    surfaces.push(
      createSurface(1, 1),
      textSurface(
        themeText(ctx, 'enter to continue  |  escape to quit', { tone: 'subdued' }),
        width,
        1
      )
    );
...
function passwordPhaseSurfaces(model, ctx, width) {
  ...
  const surfaces = [
    textSurface(
      themeText(ctx, 'Vault is encrypted. Enter passphrase to unlock.', { tone: 'warning' }),
      width,
      1
    ),
    createSurface(1, 1),
    hstackSurface(
      ...
      textSurface(`${mask}█`, Math.max(1, width - 14), 1)
    ),
  ];
  if (model.authError) {
    surfaces.push(
      ...
      hstackSurface(
        1,
        createSurface(2, 1),
        textSurface(themeText(ctx, model.authError, { tone: 'danger' }), Math.max(1, width - 3), 1)
      )
    );
  }
  surfaces.push(
    createSurface(1, 1),
    textSurface(themeText(ctx, 'enter to unlock  |  escape to quit', { tone: 'subdued' }), width, 1)
  );
  return surfaces;
}

function renderBody(model, deps, options) {
  if (model.workspace === 'atlas') {
    return renderAtlasWorkspace(model, deps, options);
  }
  if (model.workspace === 'operations') {
    return renderOperationsWorkspace(model, deps, options);
  }
...
function renderExplorerWorkspace(model, deps, options) {
  if (options.width < 88) {
    ...
    return vstackSurface(
      ...
      blank(deps.ctx, options.width, 1),
...
function renderLedgerPanel(model, ctx, box) {
  ...
  const projectedRows = model.table.rows.map((row) =>
    schema.indexes.map((index) => row[index] ?? '')
  );
...
function selectedShaFooter(model) {
  if (model.workspace !== 'explorer' || model.focusPane !== 'detail') {
    return null;
  }
...
function renderInspectorPanel(model, ctx, box) {
  ...
  return panel({
    title: 'Inspector',
    body: renderInspectorBody(entry, manifest, ctx),
    width,
    height,
    ctx,
    active: model.focusPane === 'detail',
...
function renderInspectorBody(entry, manifest, ctx) {
  ...
  lines.push(
    ...
    '',
    themeText(ctx, 'enter detail  |  tab detail focus  |  ctrl+p digest search', {
      tone: 'subdued',
    })
  );
  return lines.join('\n');
...
function renderManifestDetail({ model, manifest, ctx, box }) {
  ...
  if (chunks.length === 0) {
    return lines.concat('No chunks.').join('\n');
  }
  return lines
    .concat(
...
function renderAtlasBriefing(sidebar, ctx) {
  return [
    themeText(ctx, 'Overview', { tone: 'accent', bold: true }),
    sidebar.overview,
    '',
    themeText(ctx, 'Focused Region', { tone: 'accent', bold: true }),
    sidebar.focused,
    '',
    themeText(ctx, 'Legend', { tone: 'accent', bold: true }),
    sidebar.legend,
    '',
    themeText(ctx, 'Largest Regions', { tone: 'accent', bold: true }),
    sidebar.regions,
    '',
    sidebar.notes,
  ]
...
function renderAtlasWorkspace(model, deps, options) {
  ...
  const briefing = renderAtlasBriefing(sidebar, deps.ctx);
  return hstackSurface(
    gap,
    panel({
      title: 'Repository Atlas',
...
function renderOperationsWorkspace(model, deps, options) {
  if (options.width < 100) {
    ...
    return vstackSurface(
      ...
      renderOpsDetailPanel(model, deps.ctx, {
        width: options.width,
        height: Math.max(4, options.height - topHeight),
      })
    );
  }
  const gap = 2;
...
function renderEconomicsPanel(model, ctx, box) {
  const { width, height } = box;
  const body =
    model.statsStatus === 'ready' && model.statsReport
...
function renderStats(stats, ctx) {
  const chunking =
    Object.entries(stats.chunkingStrategies ?? {})
...
function renderOpsDetailPanel(model, ctx, box) {
  const { width, height } = box;
  const doctor =
    model.doctorStatus === 'ready' && model.doctorReport
      ? typeof model.doctorReport === 'string'
        ? model.doctorReport
...
function renderPageOverlay(model, deps, options) {
  if (model.storeWizard) {
    ...
    options.screen.blit(
      wizard,
      centerX(options.width, wizard.width),
      topThirdY(options.height, wizard.height)
    );
  }
}
